...
func DeleteRegion(c *gin.Context) {
	...
	if usage.CPU > 0 || usage.RAM > 0 || usage.Disk > 0 || usage.IPs > 0 {
		common.AbortWithDetail(c, http.StatusBadRequest, fmt.Sprintf(
			"Cannot delete region '%s': There are still active resources (VMs, volumes, or floating IPs). Please delete all resources in this region first.", region.Name))
		return
	}
