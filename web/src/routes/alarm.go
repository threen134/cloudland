...
func handleAlarmRuleError(c *macaron.Context, err error) {
	logger.Errorf("Handling alarm rule error: %v", err)
...
func (v *AlarmView) CreateNodeAlarmRule(c *macaron.Context) {
	logger.Infof("ENTER AlarmView.CreateNodeAlarmRule: query=%s", c.Req.URL.RawQuery)
	defer logger.Info("EXIT AlarmView.CreateNodeAlarmRule")
...
func (v *AlarmView) GetNodeAlarmRules(c *macaron.Context, store session.Store) {
	logger.Infof("ENTER AlarmView.GetNodeAlarmRules: query=%s", c.Req.URL.RawQuery)
	defer logger.Info("EXIT AlarmView.GetNodeAlarmRules")
	uuid := c.Query("uuid")
	ruleType := c.Query("rule_type")
	...
	if c.Locale != nil {
		...
		logger.Info("Warning: i18n object is nil")
...
func (v *AlarmView) DeleteNodeAlarmRule(c *macaron.Context) {
	logger.Infof("ENTER AlarmView.DeleteNodeAlarmRule: uuid=%s", c.Params(":uuid"))
	defer logger.Info("EXIT AlarmView.DeleteNodeAlarmRule")
	uuid := c.Params(":uuid")
	if uuid == "" {
...
func (v *AlarmView) NewNodeAlarmRule(c *macaron.Context) {
	logger.Infof("ENTER AlarmView.NewNodeAlarmRule: query=%s", c.Req.URL.RawQuery)
	defer logger.Info("EXIT AlarmView.NewNodeAlarmRule")
	c.HTML(200, "node_alarm_new")
}
