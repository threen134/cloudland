...
func (v *UserView) LoginGet(c *macaron.Context, store session.Store) {
	logger.Infof("ENTER UserView.LoginGet: query=%s", c.Req.URL.RawQuery)
	defer logger.Info("EXIT UserView.LoginGet")
...
func (v *UserView) LoginPost(c *macaron.Context, store session.Store) {
	logger.Infof("ENTER UserView.LoginPost: email=%s", c.QueryTrim("email"))
	defer logger.Info("EXIT UserView.LoginPost")
	email := c.QueryTrim("email")
	if email == "" {
		email = c.QueryTrim("username") // fallback for legacy login forms
	}
	password := c.QueryTrim("password")
	user, err := userAdmin.Validate(c.Req.Context(), email, password)
	if err != nil {
	...
	err = DB().Where("user_id = ?", uid).Find(&members).Error
	if err != nil {
...
func (v *UserView) List(c *macaron.Context, store session.Store) {
	logger.Infof("ENTER UserView.List: query=%s", c.Req.URL.RawQuery)
	defer logger.Info("EXIT UserView.List")
...
func (v *UserView) Edit(c *macaron.Context, store session.Store) {
	logger.Infof("ENTER UserView.Edit: params=%v", c.Params)
	defer logger.Info("EXIT UserView.Edit")
	memberShip := GetMemberShip(c.Req.Context())
	id := c.Params("id")
	if id == "" {
...
func (v *UserView) Change(c *macaron.Context, store session.Store) {
	logger.Infof("ENTER UserView.Change: params=%v, query=%s", c.Params, c.Req.URL.RawQuery)
	defer logger.Info("EXIT UserView.Change")
	memberShip := GetMemberShip(c.Req.Context())
	id := c.Params("id")
	if id == "" {
		c.Data["ErrorMsg"] = "Id is Empty"
	...
	orgIDStr := c.QueryTrim("org")
	redirectTo := "/dashboard"
	orgIDStr = strings.TrimSpace(orgIDStr)
	if orgIDStr != "" {
		orgID, parseErr := strconv.ParseInt(orgIDStr, 10, 64)
		if parseErr == nil {
			token, _, _, switchErr := userAdmin.SwitchOrg(c.Req.Context(), int64(userID), orgID)
			if switchErr == nil {
				store.Set("act", token)
...
func (v *UserView) Patch(c *macaron.Context, store session.Store) {
	logger.Infof("ENTER UserView.Patch: params=%v, query=%s", c.Params, c.Req.URL.RawQuery)
	defer logger.Info("EXIT UserView.Patch")
	memberShip := GetMemberShip(c.Req.Context())
	id := c.Params("id")
	if id == "" {
		c.Data["ErrorMsg"] = "Id is Empty"
	...
	password := c.QueryTrim("password")
	if password != "" {
		oldPassword := c.QueryTrim("old_password")
		err = userAdmin.ChangePassword(c.Req.Context(), int64(userID), oldPassword, password)
		if err != nil {
...
func (v *UserView) Delete(c *macaron.Context, store session.Store) (err error) {
	logger.Infof("ENTER UserView.Delete: params=%v", c.Params)
	defer logger.Info("EXIT UserView.Delete")
	ctx := c.Req.Context()
	id := c.Params("id")
	if id == "" {
...
func (v *UserView) New(c *macaron.Context, store session.Store) {
	logger.Info("ENTER UserView.New")
	defer logger.Info("EXIT UserView.New")
	memberShip := GetMemberShip(c.Req.Context())
	if !memberShip.IsSystemAdmin() {
		logger.Error("Not authorized for this operation")
		c.Data["ErrorMsg"] = "Not authorized for this operation"
...
func (v *UserView) Create(c *macaron.Context, store session.Store) {
	logger.Infof("ENTER UserView.Create: query=%s", c.Req.URL.RawQuery)
	defer logger.Info("EXIT UserView.Create")
	memberShip := GetMemberShip(c.Req.Context())
	if !memberShip.IsSystemAdmin() {
		logger.Error("Not authorized for this operation")
		c.Data["ErrorMsg"] = "Not authorized for this operation"
	...
	email := c.QueryTrim("email")
	if email == "" {
		email = c.QueryTrim("username") // fallback for legacy form fields
	}
	password := c.QueryTrim("password")
	confirm := c.QueryTrim("confirm")
	orgName := c.QueryTrim("orgname")
	...
	var err error
	if orgName != "" {
		// Create user + org together
		_, _, err = userAdmin.CreateWithOrg(c.Req.Context(), email, password, orgName, "")
	} else {
		// Create user only (Dormant, no Org — will be invited later)
		_, err = userAdmin.Create(c.Req.Context(), email, password, "")
	}
	if err != nil {
