...
const buildUsageBars = (
    ...
    const bar = (
        ...
        unit: { unitKey?: string; unit?: string }
    ): UsageBar => {
...
onMounted(async () => {
    try {
        ...
        const [instRes, volRes, imgRes, vpcRes, fipRes, lbRes, quotaRes] = await Promise.all([
            ...
            orgUuid && regionUuid
                ? quotaApi.getOrgRegionResourceInfo(orgUuid, regionUuid).catch((err) => {
        ...
        const usedCpu =
            ...
            (Array.isArray(instances) ? instances.reduce((acc: number, inst: Instance) => acc + (inst.cpu || 0), 0) : 0)
        const usedMemGB =
            consumption?.ram_gb ??
            (Array.isArray(instances)
                ? instances.reduce((acc: number, inst: Instance) => acc + (inst.memory || 0), 0) / 1024
                : 0)
        const usedDiskGB =
            consumption?.disk_gb ??
    ...
    return [
        {
            key: 'instances',
            route: 'instances',
            labelKey: 'dashboard.instances',
            icon: Server,
            bgClass: 'bg-blue-light',
            textClass: 'text-blue',
            value: s.instances.used,
            suffixKey: 'dashboard.overview.activeCount',
        },
        {
            key: 'volumes',
            route: 'volumes',
            labelKey: 'dashboard.volumes',
            icon: HardDrive,
            bgClass: 'bg-teal-light',
            textClass: 'text-teal',
            value: s.volume.used,
            suffix: s.volume.unit,
        },
        {
            key: 'floating-ips',
            route: 'floating-ips',
            labelKey: 'dashboard.floatingIPs',
            icon: Globe,
            bgClass: 'bg-blue-light',
            textClass: 'text-blue',
            value: s.public_ip.used,
            suffixKey: 'dashboard.overview.activeCount',
        },
        {
            key: 'vpcs',
            route: 'vpcs',
            labelKey: 'dashboard.vpcs',
            icon: Layers,
            bgClass: 'bg-purple-light',
            textClass: 'text-purple',
            value: s.private_ip.used,
            suffixKey: 'dashboard.overview.activeCount',
        },
        {
            key: 'load-balancers',
            route: 'load-balancers',
            labelKey: 'dashboard.loadBalancers',
            icon: Network,
            bgClass: 'bg-teal-light',
            textClass: 'text-teal',
            value: s.load_balancer.used,
            suffixKey: 'dashboard.overview.activeCount',
        },
        {
            key: 'images',
            ...
            textClass: 'text-rose',
            value: s.images.used,
            suffixKey: 'dashboard.overview.imageCount',
        },
    ]
})
    ...
    <div class="overview-page" v-if="stats">
        ...
        <div class="dashboard-grid">
                ...
                <div class="card-body">
                    <div v-for="bar in usageBars" :key="bar.key" class="usage-item">
                        ...
                        <div class="progress-bg">
                            <div
                                class="progress-fill"
                                :style="{ width: bar.percentage + '%', backgroundColor: getBarColor(bar) }"
                            ></div>
                        </div>
                        <div class="usage-meta">
                            {{ bar.used }} / {{ bar.total !== null ? bar.total : '-' }}
                            {{ bar.unitKey ? $t(bar.unitKey) : bar.unit }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card activity-card">
                ...
                <div class="card-body">
                    <div v-if="activitiesLoading" class="activity-empty">
                        {{ $t('dashboard.overview.activityLoading') }}
                    </div>
                    <div v-else-if="activitiesError" class="activity-empty">
                        {{ $t('dashboard.overview.activityLoadFailed') }}
                        <button class="activity-retry" @click="loadActivities">
                            {{ $t('dashboard.overview.activityRetry') }}
                        </button>
                    </div>
                    <div v-else-if="activities.length === 0" class="activity-empty">
                        {{ $t('dashboard.overview.activityEmpty') }}
                    </div>
