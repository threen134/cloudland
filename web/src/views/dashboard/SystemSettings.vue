                    ...
                    <div class="pane-content">
                        ...
                        <div class="settings-form-body" :class="{ disabled: !enabledChannels.includes(activeChannelTab) }">
                            <div v-for="setting in getSettingsByChannel(activeChannelTab)" :key="setting.key" class="form-row">
                                <div class="form-info">
                                    <label class="item-label">{{ getFieldLabel(setting.key) }}</label>
                                ...
                                <div class="form-control">
                                    <div v-if="setting.value_type === 'boolean'" class="bool-wrap">
                                        <label class="custom-toggle sm">
                                            <input type="checkbox" :disabled="!enabledChannels.includes(activeChannelTab)" :checked="!!editValues[setting.key]" @change="editValues[setting.key] = ($event.target as HTMLInputElement).checked" />
                                            <span class="toggle-slider"></span>
                                        </label>
                                    ...
                                    <div v-else-if="setting.value_type === 'number'" class="input-wrap">
                                        <input type="number" class="form-input" :disabled="!enabledChannels.includes(activeChannelTab)" v-model.number="editValues[setting.key]" />
                                    </div>
                                    <div v-else-if="setting.value_type === 'secret'" class="input-wrap">
                                        <input type="password" class="form-input" :disabled="!enabledChannels.includes(activeChannelTab)" v-model="editValues[setting.key]" :placeholder="$t('settings.secretPlaceholder')" autocomplete="new-password" />
                                        <Shield class="icon-inner" :size="16" />
                                    </div>
                                    <div v-else-if="setting.value_type === 'json'" class="input-wrap">
                                        <textarea class="form-input code-area" :disabled="!enabledChannels.includes(activeChannelTab)" v-model="editValues[setting.key]" rows="4" spellcheck="false" />
                                        <div class="type-tag">JSON</div>
                                    </div>
                                    <div v-else class="input-wrap">
                                        <input type="text" class="form-input" :disabled="!enabledChannels.includes(activeChannelTab)" v-model="editValues[setting.key]" />
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="settings-form-body">
                        <div v-for="setting in currentSettings" :key="setting.key" class="form-row">
                            ...
                            <div class="form-control">
                                <div v-if="setting.value_type === 'boolean'" class="bool-wrap">
                                    <label class="custom-toggle sm">
                                        <input type="checkbox" :checked="!!editValues[setting.key]" @change="editValues[setting.key] = ($event.target as HTMLInputElement).checked" />
                                        <span class="toggle-slider"></span>
                                    </label>
                                    <span class="bool-status-text">{{ !!editValues[setting.key] ? $t('common.on') : $t('common.off') }}</span>
                                </div>
                                <div v-else-if="setting.value_type === 'number'" class="input-wrap">
                                    <input type="number" class="form-input" v-model.number="editValues[setting.key]" />
                                </div>
                                <div v-else-if="setting.value_type === 'secret'" class="input-wrap">
                                    <input type="password" class="form-input" v-model="editValues[setting.key]" :placeholder="$t('settings.secretPlaceholder')" autocomplete="new-password" />
                                    <Shield class="icon-inner" :size="16" />
                                </div>
                                <div v-else-if="setting.value_type === 'json'" class="input-wrap">
                                    <textarea class="form-input code-area" v-model="editValues[setting.key]" rows="6" spellcheck="false" />
                                    <div class="type-tag">JSON</div>
                                </div>
                                <div v-else class="input-wrap">
                                    <input type="text" class="form-input" v-model="editValues[setting.key]" />
                                </div>
                            </div>
                        </div>
                    </div>
