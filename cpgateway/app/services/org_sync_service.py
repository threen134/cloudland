...
class OrgSyncService:
    ...
    @staticmethod
    async def sync_org_to_all_regions(db: AsyncSession, org: Organization):
        """将单个 org 同步到所有活跃 Region（创建/更新 org 时调用）"""
        ...
        payload = {"id": org.id, "name": org.name, "slug": org.slug or ""}
    ...
    async def sync_all_orgs_to_region(db: AsyncSession, region: Region):
        ...
        async with httpx.AsyncClient(verify=False, timeout=10.0) as client:
            for org in orgs:
                payload = {"id": org.id, "name": org.name, "slug": org.slug or ""}
                try:
                    await OrgSyncService._push_to_region(region, payload, client)
                except Exception as e:
                    logger.error(
                        f"Failed to sync org '{org.name}' (id={org.id}) "
                        f"to new region '{region.name}': {e}"
                    )
        logger.info(
