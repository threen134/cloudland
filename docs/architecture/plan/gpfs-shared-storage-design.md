...
- **执行顺序**：`local-multi-disk-storage-plan.md`（本地多盘存储池）**先于本文实施**。本文阶段 0 中与本地存储有关的部分由它的阶段 L0a / L0b 完成（`storage_pools`、`hyper_storage_pools` 和存储池接口也由它先建出来），本文在其基础上增加 `gpfs` 驱动，见 §14 阶段 0 的说明。两份文档对同一张表、同一个脚本的描述不一致时，**以本地方案为准**，本文只增加 GPFS 专用的部分（§3.3、§4.1、§4.2、§5.1、§6.2、§6.4、§6.9、§7、§9.2、§10.1、§11.1、§14 已加注）
...
> **已决定放弃 WDS**（2026-09-22），并已随本地多盘方案的实施整体删除：WDS 分支、`*_wds*.sh`、`xml/wds_*.xml`、备份与一致性组（只支持 WDS）、卷 QoS、`tools/`、Ansible 的 `wds` 角色、相关接口与代理白名单。本节以下内容留作决策记录，其中「如果决定保留」的分支不再适用。
...
> **以本地方案 §3.1 为准**：这张表由本地方案 L0a 先建出来，L1 增加 `Media`、`FallbackGroup`。池级的 `CapacityBytes`、`UsedBytes`、`CapacityAt` 只对 GPFS 池有意义，执行本文时再加；本地池的池级容量按各节点记录查询时汇总。`Status=disabled` 对本地池同样适用。删除存储池时，本地池要求没有任何节点记录；GPFS 池的节点可用性记录随池一起删除，不作为拒绝删除的理由。
...
> **以本地方案 §3.2 为准**：这张表由本地方案 L1 先建出来，本地池（包括内置池）同样有记录，并且多出布局、盘、容量等字段，以及 `creating` / `extending` / `removing` / `degraded` / `maintenance` / `lost` / `error` 等状态。GPFS 池的记录只用其中的 `ready` / `unavailable`，其余字段留空。原先「本地池不需要这张表」的说法作废。删除节点时：这台节点上有非内置本地池的记录或本地卷时拒绝删除，除非选择「保留池以便接管」（本地方案 §5.9）；内置池记录和 GPFS 池的记录随节点一起删除。
...
> **以本地方案 §2.3、§4.6 为准**：`pool_guard` 由本地方案 L1 先建，签名为 `pool_guard <根目录> <池UUID> <期望的hostid>`；执行本文时增加驱动参数，GPFS 池按上面第 1 条检查文件系统类型、不做 `mountpoint` 检查、不校验 hostid。标记文件统一为 key=value 格式：本地池为 `driver`、`pool_uuid`、`hostid` 三行，GPFS 池只有 `driver=gpfs`、`pool_uuid` 两行（共享池没有归属节点），上面第 2 条相应改为「`pool_uuid` 等于池的 UUID」。
...
> **以本地方案 §5.5 为准**：本地卷的删除已改为「置为 `deleting` → 节点删除文件后回调 `clear_volume` → 软删记录」，超时转 `delete_failed`，接口返回 202。GPFS 卷沿用同一套状态和回调，参数顺序也统一为 `clear_volume_<驱动>.sh '<卷ID>' '<卷UUID>' '<绝对路径>' '<池UUID>' '<hostid>'`（GPFS 卷的 hostid 为执行删除的节点）。
...
> **以本地方案 §4.8 为准**：不在 `sync_instance` 里等待。`report_rc.sh` 就是心跳，在里面等几分钟会让节点被判离线（WDS 等待设备就曾因此让节点离线约 7 分钟）。本地方案改为「待启动列表」：磁盘所在的池没有就绪的虚拟机先不启动，之后每次心跳检查，池就绪了再启动并补发同步回调。GPFS 池沿用这套机制，只是「池是否就绪」按 GPFS 的检查判断。
...
> **本地盘的迁移以本地方案 §7 为准**：
> - 源端按迁移计划（`migrations.DiskPlan`）处理磁盘，而不是按卷 JSON；计划在目标机器确定后生成并准入。
> - 目标端的挂载检查、按实际数据量的空间预检、预建（沿用源盘的簇大小）都由源端经 ssh 完成。
> - 关机迁移改用 `rsync --sparse`。
> - 维护模式批量迁移改为逐台处理、逐台返回结果。
> - 调度时的磁盘需求只计内置池里的系统盘（本地方案 §5.6）。
>
> 下文只在此基础上增加 GPFS 盘的处理：共享盘不复制、不预建、不清理。
...
> **与本地方案 §6.1 的关系**：本地盘用 `error_policy='enospace'`，空间不足时暂停，空间回落后由节点自动恢复。自动恢复只针对 `virsh domblkerror` 显示「出错的盘都是本地盘、原因都是空间不足」的虚拟机；因 GPFS 盘出错而暂停的虚拟机不会被判为存储空间不足，也不会被自动恢复。
...
> **以本地方案 §8.1 为准**：存储池接口由本地方案 L1 先建出来。普通成员在上表 5 个字段之外，还能看到 `media` 和可用机器数。删除存储池的条件按上面 §4.1 的注释。节点相关的子路由在 clapi 里统一用 `/hypers/:uuid/...`，网关白名单里写作 `/hypers/:hostid/...`，传的都是节点 UUID。
...
- 节点开机时，要等 GPFS 挂载好再启动相关虚拟机，由本地方案 §4.8 的待启动列表保证（见 §6.9 的注释），不依赖 systemd 的启动顺序。
...
> **执行顺序调整**：本地多盘方案（`local-multi-disk-storage-plan.md`）先实施。它的阶段 L0a（数据盘部分）与 L0b（系统盘、救援、重装、捕获镜像、WDS 清理）合起来完成下面范围里除 GPFS 专用部分以外的全部内容。附录 B 的分工：
>
> - 第 1、2、5、7、8、9 条在 L0b 修。
> - 第 3 条的前半（`du -x`）在 L0a 做。
> - 第 6 条的路径部分在 L0a 做。
> - 第 3 条的后半（NVRAM 被当 GiB 解析）和第 4 条（磁盘需求单位）会改变调度结果，在本地方案里列为单独评估（本地方案附录 B 第 7、6 条），不随阶段 0 顺带修。
>
> 它的 L1 还会建好 `hyper_storage_pools`、存储池接口与 `pool_guard`，并给 `storage_pools` 增加 `Media`、`FallbackGroup` 字段。执行本文时，阶段 0 只需补上 GPFS 专用的字段（`FsName`、`Fileset`、`FsType`、`CloneMode`，以及 `volumes.BaseImageStorageID`）和 `image_storages` 的改造，再从阶段 1 开始。
...
| 10 | WDS 的去留（C1） | 阶段 0 的范围 | **已决定放弃并删除**（2026-09-22，见 §1.4） |
