...
### 4. 进阶配置与运维
- [配置参考](./05-configuration.md) — 环境变量、目录结构及其自定义方法。
- [运维管理](./06-operations.md) — 常用容器指令、故障排查技巧及日常维护流程。

---

## 部署模式说明

CloudLand 通过 `COMPOSE_PROFILES` 环境变量控制启用的组件。三个 Profile 可自由组合：
...
graph TB
    ...
    subgraph regionA["Region A <small>（区域控制面）</small>"]
        ...
        CONSOLE_A["consoleproxy<br/><small>VNC :9443</small>"]
    ...
    subgraph regionB["Region B <small>（区域控制面）</small>"]
        ...
        CONSOLE_B["consoleproxy<br/><small>VNC :9443</small>"]
...
> [!NOTE]
> **单 Region 部署**时，中央控制面与区域控制面部署在同一台机器上（即 `COMPOSE_PROFILES=full,dev,region`）。
> **多 Region 部署**时，中央控制面独立部署（`full,dev`），各区域只部署核心服务（`region`）。
...
| **仅区域控制面** | `region` | cloudland + clapi + consoleproxy + 监控栈 | 远程 Region 节点，无认证/UI，对接中央控制面 |
...
如果您在部署过程中遇到任何问题，请参考 [故障排查](./06-operations.md#故障排查) 或前往我们的 [GitHub 项目页](https://github.com/threen134/cloudland) 提交 Issue。
