...
func (a *AddressAdmin) GetAddressByUUID(ctx context.Context, uuID string) (addr *model.Address, err error) {
	logger.Ctx(ctx).Infof("ENTER AddressAdmin.GetAddressByUUID: uuID=%s", uuID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT AddressAdmin.GetAddressByUUID: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT AddressAdmin.GetAddressByUUID: success, addrID=%d", addr.ID)
		}
	}()
...
func (a *AddressAdmin) Update(ctx context.Context, addr *model.Address) (err error) {
	logger.Ctx(ctx).Infof("ENTER AddressAdmin.Update: addrID=%d", addr.ID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT AddressAdmin.Update: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT AddressAdmin.Update: success")
		}
	}()
...
func (a *AddressAdmin) ListBySubnetID(ctx context.Context, subnetID int64) (addresses []*model.Address, err error) {
	logger.Ctx(ctx).Infof("ENTER AddressAdmin.ListBySubnetID: subnetID=%d", subnetID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT AddressAdmin.ListBySubnetID: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT AddressAdmin.ListBySubnetID: count=%d", len(addresses))
		}
	}()
	ctx, db := GetContextDB(ctx)
	...
	if err != nil {
		logger.Ctx(ctx).Error("Failed to query addresses for subnet, %v", err)
