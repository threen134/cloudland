...
func createRouterIface(ctx context.Context, rtype string, router *model.Router, owner int64) (iface *model.Interface, subnet *model.Subnet, err error) {
	...
	if err != nil {
		logger.Ctx(ctx).Error("Failed to query subnets", err)
	...
	for _, subnet = range subnets {
		...
		if err == nil {
			logger.Ctx(ctx).Error("Created gateway interface from subnet")
			break
		}
	}
	return
...
func (a *RouterAdmin) Create(ctx context.Context, name, description string) (router *model.Router, err error) {
	logger.Ctx(ctx).Infof("ENTER RouterAdmin.Create: name=%s", name)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT RouterAdmin.Create: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT RouterAdmin.Create: success")
		}
	}()
	...
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to create router ", err)
		err = NewCLError(ErrRouterCreateFailed, "Failed to create router", err)
		return
	}
	secGroup, err := secgroupAdmin.Create(ctx, name+"-native", "Native security group for router "+name, true, true, router)
	if err != nil {
		logger.Ctx(ctx).Error("Failed to create security group", err)
		return
	}
...
func (a *RouterAdmin) Get(ctx context.Context, id int64) (router *model.Router, err error) {
	logger.Ctx(ctx).Infof("ENTER RouterAdmin.Get: id=%d", id)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT RouterAdmin.Get: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT RouterAdmin.Get: success")
		}
	}()
	if id <= 0 {
	...
	if !permit {
		logger.Ctx(ctx).Error("Not authorized to read the router")
...
func (a *RouterAdmin) GetRouterByUUID(ctx context.Context, uuID string) (router *model.Router, err error) {
	logger.Ctx(ctx).Infof("ENTER RouterAdmin.GetRouterByUUID: uuID=%s", uuID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT RouterAdmin.GetRouterByUUID: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT RouterAdmin.GetRouterByUUID: success")
		}
	}()
	ctx, db := GetContextDB(ctx)
	memberShip := GetMemberShip(ctx)
...
func (a *RouterAdmin) GetRouterByName(ctx context.Context, name string) (router *model.Router, err error) {
	logger.Ctx(ctx).Infof("ENTER RouterAdmin.GetRouterByName: name=%s", name)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT RouterAdmin.GetRouterByName: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT RouterAdmin.GetRouterByName: success")
		}
	}()
	ctx, db := GetContextDB(ctx)
	memberShip := GetMemberShip(ctx)
...
func (a *RouterAdmin) GetRouter(ctx context.Context, reference *BaseReference) (router *model.Router, err error) {
	logger.Ctx(ctx).Infof("ENTER RouterAdmin.GetRouter: reference=%+v", reference)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT RouterAdmin.GetRouter: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT RouterAdmin.GetRouter: success")
		}
	}()
...
func (a *RouterAdmin) Update(ctx context.Context, id int64, name string, description *string, pubID int64) (router *model.Router, err error) {
	logger.Ctx(ctx).Infof("ENTER RouterAdmin.Update: id=%d, name=%s, description=%v, pubID=%d", id, name, description, pubID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT RouterAdmin.Update: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT RouterAdmin.Update: success")
		}
	}()
	ctx, db := GetContextDB(ctx)
	router = &model.Router{Model: model.Model{ID: id}}
	...
	if len(updates) > 0 {
		if err = db.Model(router).Updates(updates).Error; err != nil {
			logger.Ctx(ctx).Error("Failed to update router", err)
...
func (a *RouterAdmin) Delete(ctx context.Context, router *model.Router) (err error) {
	logger.Ctx(ctx).Infof("ENTER RouterAdmin.Delete: routerID=%d, uuid=%s", router.ID, router.UUID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT RouterAdmin.Delete: error=%v", err)
		} else {
	...
	if count > 0 {
		logger.Ctx(ctx).Error("There are floating ips")
	...
	if err != nil {
		logger.Ctx(ctx).Error("Failed to count subnet")
		err = NewCLError(ErrDatabaseError, "Failed to count subnet in the router", err)
		return
	}
	if count > 0 {
		logger.Ctx(ctx).Error("There are associated subnets")
	...
	if err != nil {
		logger.Ctx(ctx).Error("Failed to count portmap")
		err = NewCLError(ErrDatabaseError, "Failed to count portmap in the router", err)
		return
	}
	if count > 0 {
		logger.Ctx(ctx).Error("There are associated portmaps")
	...
	if err != nil {
		logger.Ctx(ctx).Error("Delete master failed")
		return
	}
	if router.VrrpSubnetID > 0 {
		...
		if err != nil {
			logger.Ctx(ctx).Error("Failed to get vrrp subnet", err)
			return
		}
	...
	if err = db.Delete(router).Error; err != nil {
		logger.Ctx(ctx).Error("DB failed to delete router", err)
	...
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to update router name", err)
	...
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to query security groups", err)
	...
	for _, sg := range secgroups {
		...
		if err != nil {
			logger.Ctx(ctx).Error("Can not delete security group", err)
...
func (a *RouterAdmin) List(ctx context.Context, offset, limit int64, order, query string) (total int64, routers []*model.Router, err error) {
	logger.Ctx(ctx).Infof("ENTER RouterAdmin.List: offset=%d, limit=%d, order=%s, query=%s", offset, limit, order, query)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT RouterAdmin.List: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT RouterAdmin.List: success")
		}
	}()
	memberShip := GetMemberShip(ctx)
