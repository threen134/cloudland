...
func (a *BackendAdmin) CreateHaproxyConf(ctx context.Context, updatedlistener *model.Listener, loadBalancer *model.LoadBalancer) (err error) {
	logger.Ctx(ctx).Infof("ENTER BackendAdmin.CreateHaproxyConf: loadBalancerID=%d", loadBalancer.ID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackendAdmin.CreateHaproxyConf: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT BackendAdmin.CreateHaproxyConf: success")
		}
	}()
	...
	if err != nil {
		logger.Ctx(ctx).Errorf("Failed to marshal load balancer json data, %v", err)
		return
	}
	hyperGroup, _, _, err := GetVrrpHyperGroup(ctx, loadBalancer.VrrpInstance)
	if err != nil {
		logger.Ctx(ctx).Errorf("Failed to get hyper group, %v", err)
		return
	}
...
func (a *BackendAdmin) Create(ctx context.Context, name, backendAddr string, ssl bool, listener *model.Listener, loadBalancer *model.LoadBalancer) (backend *model.Backend, err error) {
	logger.Ctx(ctx).Infof("ENTER BackendAdmin.Create: name=%s, backendAddr=%s, listenerID=%d, loadBalancerID=%d", name, backendAddr, listener.ID, loadBalancer.ID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackendAdmin.Create: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackendAdmin.Create: success, backendID=%d", backend.ID)
		}
	}()
	memberShip := GetMemberShip(ctx)
	permit := memberShip.CheckOrgPermission(model.OrgWriter)
	if !permit {
		logger.Ctx(ctx).Error("Not authorized to create backend")
	...
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to create backend ", err)
...
func (a *BackendAdmin) Get(ctx context.Context, id int64, listener *model.Listener) (backend *model.Backend, err error) {
	logger.Ctx(ctx).Infof("ENTER BackendAdmin.Get: id=%d, listenerID=%d", id, listener.ID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackendAdmin.Get: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackendAdmin.Get: success, backendUUID=%s", backend.UUID)
		}
	}()
	if id <= 0 {
		logger.Ctx(ctx).Error("returning nil backend")
		return
	}
	...
	if err = db.Where(query, args...).Take(backend).Error; err != nil {
		logger.Ctx(ctx).Error("Failed to query backend", err)
	...
	if !permit {
		logger.Ctx(ctx).Error("Not authorized to read the backend")
...
func (a *BackendAdmin) GetBackendByUUID(ctx context.Context, uuID string) (backend *model.Backend, err error) {
	logger.Ctx(ctx).Infof("ENTER BackendAdmin.GetBackendByUUID: uuID=%s", uuID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackendAdmin.GetBackendByUUID: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackendAdmin.GetBackendByUUID: success, id=%d", backend.ID)
		}
	}()
	ctx, db := GetContextDB(ctx)
	memberShip := GetMemberShip(ctx)
	...
	if err != nil {
		logger.Ctx(ctx).Error("Failed to query backend, %v", err)
...
func (a *BackendAdmin) GetBackendByName(ctx context.Context, name string) (backend *model.Backend, err error) {
	logger.Ctx(ctx).Infof("ENTER BackendAdmin.GetBackendByName: name=%s", name)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackendAdmin.GetBackendByName: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackendAdmin.GetBackendByName: success, id=%d", backend.ID)
		}
	}()
	ctx, db := GetContextDB(ctx)
	memberShip := GetMemberShip(ctx)
...
func (a *BackendAdmin) GetBackend(ctx context.Context, reference *BaseReference) (backend *model.Backend, err error) {
	logger.Ctx(ctx).Infof("ENTER BackendAdmin.GetBackend: reference=%v", reference)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackendAdmin.GetBackend: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT BackendAdmin.GetBackend: success")
		}
	}()
...
func (a *BackendAdmin) Delete(ctx context.Context, backend *model.Backend, listener *model.Listener, loadBalancer *model.LoadBalancer) (err error) {
	logger.Ctx(ctx).Infof("ENTER BackendAdmin.Delete: backendID=%d, listenerID=%d, loadBalancerID=%d", backend.ID, listener.ID, loadBalancer.ID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackendAdmin.Delete: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT BackendAdmin.Delete: success")
		}
	}()
	ctx, db, newTransaction := StartTransaction(ctx)
	defer func() {
	...
	permit := memberShip.CheckResourceOrg(model.OrgWriter, backend.Owner)
	if !permit {
		logger.Ctx(ctx).Error("Not authorized to delete the backend")
	...
	err = a.CreateHaproxyConf(ctx, listener, loadBalancer)
	if err != nil {
		logger.Ctx(ctx).Error("Failed to delete haproxy conf ", err)
...
func (a *BackendAdmin) List(ctx context.Context, offset, limit int64, order string, listener *model.Listener) (total int64, backends []*model.Backend, err error) {
	logger.Ctx(ctx).Infof("ENTER BackendAdmin.List: offset=%d, limit=%d, order=%s, listenerID=%d", offset, limit, order, listener.ID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackendAdmin.List: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackendAdmin.List: total=%d, count=%d", total, len(backends))
		}
	}()
	memberShip := GetMemberShip(ctx)
	ctx, db := GetContextDB(ctx)
	...
	if err = db.Where(where).Where(query, args...).Find(&backends).Error; err != nil {
		logger.Ctx(ctx).Error("DB failed to query backends, %v", err)
