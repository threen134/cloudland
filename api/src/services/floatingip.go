...
func AllocateFloatingIp(ctx context.Context, fipID, orgID int64, subnets []*model.Subnet, publicIp string) (*model.Interface, error) {
	logger.Ctx(ctx).Infof("ENTER AllocateFloatingIp: fipID=%d, orgID=%d, publicIp=%s", fipID, orgID, publicIp)
	defer func() {
		logger.Ctx(ctx).Info("EXIT AllocateFloatingIp")
	}()
	...
	for _, subnet := range subnets {
		...
		if err == nil {
			logger.Ctx(ctx).Infof("Successfully allocated floating IP from subnet %s", subnet.Name)
			return iface, nil
		}
		logger.Ctx(ctx).Debugf("Failed to allocate from subnet %s: %v", subnet.Name, err)
	}

	if err != nil {
		logger.Ctx(ctx).Errorf("Failed to allocate floating IP from all subnets: %v", err)
...
func (a *FloatingIpAdminService) DeallocateFloatingIp(ctx context.Context, fipID int64) error {
	logger.Ctx(ctx).Infof("ENTER DeallocateFloatingIp: fipID=%d", fipID)
	defer func() {
		logger.Ctx(ctx).Info("EXIT DeallocateFloatingIp")
	}()
	...
	if err != nil {
		logger.Ctx(ctx).Errorf("Failed to query floating IP: %v", err)
	...
	if fip.Interface != nil {
		...
		if err != nil {
			logger.Ctx(ctx).Errorf("Failed to delete interface for floating IP: %v", err)
	...
	if err != nil {
		logger.Ctx(ctx).Errorf("Failed to delete floating IP record: %v", err)
		return NewCLError(ErrFIPDeleteFailed, "Failed to delete floating IP", err)
	}

	logger.Ctx(ctx).Infof("Successfully deallocated floating IP %d", fipID)
	return nil
}
...
func (a *FloatingIpAdminService) createAndAllocateFloatingIps(ctx context.Context, name string, inbound, outbound int32, count int, subnets []*model.Subnet, publicIp string, instance *model.Instance, isSite bool, group *model.IpGroup, loadBalancer *model.LoadBalancer) ([]*model.FloatingIp, error) {
	...
	logger.Ctx(ctx).Debugf("subnets: %v, publicIp: %s, instance: %v, count: %d, inbound: %d, outbound: %d", subnets, publicIp, instance, count, inbound, outbound)
	for i := 0; i < count; i++ {
		...
		if err := db.Create(fip).Error; err != nil {
			logger.Ctx(ctx).Error("DB failed to create floating ip", err)
			return nil, NewCLError(ErrFIPCreateFailed, "Failed to create floating ip", err)
		}
		logger.Ctx(ctx).Debugf("fip: %v, subnets: %v, publicIp: %s", fip, subnets, publicIp)
		fipIface, err := AllocateFloatingIp(ctx, fip.ID, memberShip.OrgID, subnets, publicIp)
		if err != nil {
			logger.Ctx(ctx).Error("DB failed to allocate floating ip", err)
			return nil, err
		}
		...
		if instance != nil {
			if err := a.Attach(ctx, fip, instance); err != nil {
				logger.Ctx(ctx).Error("Execute attaching floating ip failed", err)
				return nil, err
			}
		}
		if isSite {
				...
				for _, subnet := range subnets {
					if subnet.Interface != primaryInterfaceID {
						...
						if err := db.Model(subnet).Update("interface", primaryInterfaceID).Error; err != nil {
							logger.Ctx(ctx).Error("Failed to update subnet interface", err)
		...
		if err := db.Model(&model.FloatingIp{}).Where("id = ?", fip.ID).Updates(map[string]interface{}{"fip_address": fip.FipAddress, "ip_address": fip.IPAddress, "subnet_id": fip.SubnetID, "type": fip.Type}).Error; err != nil {
			logger.Ctx(ctx).Error("DB failed to update floating ip", err)
...
func (a *FloatingIpAdminService) Create(ctx context.Context, instance *model.Instance, pubSubnets []*model.Subnet, publicIp string, name string, inbound, outbound, activationCount int32, siteSubnets []*model.Subnet, group *model.IpGroup, loadBalancer *model.LoadBalancer) (floatingIps []*model.FloatingIp, err error) {
	logger.Ctx(ctx).Infof("ENTER FloatingIpAdmin.Create: name=%s, publicIp=%s, instanceID=%v", name, publicIp, instance)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT FloatingIpAdmin.Create: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT FloatingIpAdmin.Create: success")
		}
	}()
	memberShip := GetMemberShip(ctx)
	...
	logger.Ctx(ctx).Debugf("instance: %v, pubSubnets: %v, publicIp: %s, name: %s, inbound: %d, outbound: %d, activationCount: %d, siteSubnets: %v", instance, pubSubnets, publicIp, name, inbound, outbound, activationCount, siteSubnets)

	if publicIp != "" && (activationCount > 1 || len(siteSubnets) > 0) {
		logger.Ctx(ctx).Error("Public ip and subnets cannot be specified at the same time")
	...
	if len(pubSubnets) == 0 {
		...
		if err != nil {
			logger.Ctx(ctx).Error("Failed to query public subnets ", err)
			return nil, NewCLError(ErrSQLSyntaxError, "Failed to query public subnets", err)
		}
		if len(pubSubnets) == 0 {
			logger.Ctx(ctx).Error("No public subnets available")
	...
	for _, subnet := range pubSubnets {
		if subnet.Type != "public" {
			logger.Ctx(ctx).Error("Subnet must be public", err)
	...
	if idleCountTotal < int64(activationCount) {
		logger.Ctx(ctx).Errorf("Not enough idle addresses for public subnets, idleCountTotal: %d, activationCount: %d, pubSubnets: %v", idleCountTotal, activationCount, pubSubnets)
	...
	if len(pubSubnets) > 0 {
		...
		logger.Ctx(ctx).Debugf("Sorted pubSubnets by priority: %v", pubSubnets)
	}

	if len(siteSubnets) > 0 {
		for _, subnet := range siteSubnets {
			if subnet.Type != "site" {
				logger.Ctx(ctx).Error("Subnet must be site", err)
			...
			if idleCount == 0 {
				logger.Ctx(ctx).Errorf("No idle addresses for site subnet %s", subnet.Name)
	...
	floatingIps = make([]*model.FloatingIp, 0)
	logger.Ctx(ctx).Debugf("pubSubnets: %v, publicIp: %s, instance: %v, activationCount: %d, inbound: %d, outbound: %d, siteSubnets: %v, group: %v", pubSubnets, publicIp, instance, activationCount, inbound, outbound, siteSubnets, group)
	...
	logger.Ctx(ctx).Debugf("siteSubnets: %v", siteSubnets)
	for i := 0; i < len(siteSubnets); i++ {
		logger.Ctx(ctx).Debugf("siteSubnets[%d]: %v, idleCount: %d, activationCount: %d, inbound: %d, outbound: %d, group: %v", i, siteSubnets[i], siteSubnets[i].IdleCount, siteSubnets[i].IdleCount, inbound, outbound, group)
...
func (a *FloatingIpAdminService) Attach(ctx context.Context, floatingIp *model.FloatingIp, instance *model.Instance) (err error) {
	logger.Ctx(ctx).Infof("ENTER FloatingIpAdmin.Attach: floatingIpID=%d, instanceID=%d", floatingIp.ID, instance.ID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT FloatingIpAdmin.Attach: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT FloatingIpAdmin.Attach: success")
		}
	}()
	if floatingIp.Type != string(PublicFloating) && floatingIp.Type != string(PublicSite) {
		logger.Ctx(ctx).Infof("Cannot attach floating IP of type %s, only PublicFloating and PublicSite types are supported for attachment", floatingIp.Type)
...
func (a *FloatingIpAdminService) Get(ctx context.Context, id int64) (floatingIp *model.FloatingIp, err error) {
	logger.Ctx(ctx).Infof("ENTER FloatingIpAdmin.Get: id=%d", id)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT FloatingIpAdmin.Get: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT FloatingIpAdmin.Get: success")
		}
	}()
	if id <= 0 {
	...
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to query floatingIp ", err)
	...
	if floatingIp.InstanceID > 0 {
		...
		if err != nil {
			logger.Ctx(ctx).Error("Failed to get ip load balancer ", err)
			return
		}
	}
...
func (a *FloatingIpAdminService) GetFloatingIpByUUID(ctx context.Context, uuID string) (floatingIp *model.FloatingIp, err error) {
	logger.Ctx(ctx).Infof("ENTER FloatingIpAdmin.GetFloatingIpByUUID: uuID=%s", uuID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT FloatingIpAdmin.GetFloatingIpByUUID: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT FloatingIpAdmin.GetFloatingIpByUUID: success")
		}
	}()
	ctx, db := GetContextDB(ctx)
	memberShip := GetMemberShip(ctx)
	...
	if err != nil {
		logger.Ctx(ctx).Error("Failed to query floatingIp, %v", err)
	...
	if floatingIp.InstanceID > 0 {
		...
		floatingIp.LoadBalancer, err = loadBalancerAdmin.Get(ctx, floatingIp.LoadBalancerID)
		if err != nil {
			logger.Ctx(ctx).Error("DB failed to query load balancer ", err)
...
func (a *FloatingIpAdminService) Detach(ctx context.Context, floatingIp *model.FloatingIp) (err error) {
	logger.Ctx(ctx).Infof("ENTER FloatingIpAdmin.Detach: floatingIpID=%d", floatingIp.ID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT FloatingIpAdmin.Detach: error=%v", err)
		} else {
	...
	if floatingIp.Type == string(PublicNative) {
		if err = db.Delete(floatingIp).Error; err != nil {
			logger.Ctx(ctx).Error("DB: delete native fip failed", err)
	...
	if floatingIp.Instance != nil {
		...
		if err != nil {
			logger.Ctx(ctx).Error("Failed to query vrrp hyper group and interfaces", err)
			return
		}
	...
	logger.Ctx(ctx).Debugf("Floating ip: %v\n", floatingIp)
	floatingIp.InstanceID = 0
	floatingIp.Instance = nil
	...
	if err != nil {
		logger.Ctx(ctx).Error("Failed to update instance ID for floating ip", err)
	...
	if loadBalancer != nil {
		...
		if err != nil {
			logger.Ctx(ctx).Error("Failed to list floating ip(s), %v", err)
			return
		}
		err = CreateVrrpConf(ctx, loadBalancer)
		if err != nil {
...
func (a *FloatingIpAdminService) Update(ctx context.Context, floatingIp *model.FloatingIp, instance *model.Instance, group *model.IpGroup, loadBalancer *model.LoadBalancer) (floatingIpTemp *model.FloatingIp, err error) {
	...
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT FloatingIpAdmin.Update: error=%v", err)
		} else {
	...
	if group != nil {
		...
		if err != nil {
			logger.Ctx(ctx).Error("Failed to update floating ip group_id", err)
	...
	if err != nil {
		logger.Ctx(ctx).Error("Failed to get updated floating ip", err)
		return
	}
...
func (a *FloatingIpAdminService) Delete(ctx context.Context, floatingIp *model.FloatingIp) (err error) {
	logger.Ctx(ctx).Infof("ENTER FloatingIpAdmin.Delete: floatingIpID=%d, uuid=%s", floatingIp.ID, floatingIp.UUID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT FloatingIpAdmin.Delete: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT FloatingIpAdmin.Delete: success")
		}
	}()
	...
	if floatingIp.Instance != nil || floatingIp.LoadBalancer != nil {
		err = a.Detach(ctx, floatingIp)
		if err != nil {
			logger.Ctx(ctx).Error("Failed to detach floating ip", err)
			return
		}
	}
	err = a.DeallocateFloatingIp(ctx, floatingIp.ID)
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to deallocate floating ip", err)
		return
	}
	return
}
...
func (a *FloatingIpAdminService) List(ctx context.Context, offset, limit int64, order, query string, intQuery string) (total int64, floatingIps []*model.FloatingIp, err error) {
	...
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT FloatingIpAdmin.List: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT FloatingIpAdmin.List: success")
		}
	}()
	memberShip := GetMemberShip(ctx)
	...
	for _, fip := range floatingIps {
		...
		if fip.InstanceID > 0 {
			...
			if err != nil {
				logger.Ctx(ctx).Error("Failed to query interfaces ", err)
				err = nil
				continue
			}
