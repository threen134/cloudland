...
func (a *BackupAdmin) CreateBackupByID(ctx context.Context, volumeID int64, poolID string, name string) (backup *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.CreateBackupByID: volumeID=%d, poolID=%s, name=%s", volumeID, poolID, name)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.CreateBackupByID: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.CreateBackupByID: success, backupID=%d", backup.ID)
		}
	}()
	volume, err := volumeAdmin.Get(ctx, volumeID)
	if err != nil {
		logger.Ctx(ctx).Error("Failed to get volume", err)
		return
	}
...
func (a *BackupAdmin) CreateBackupByUUID(ctx context.Context, uuid string, poolID string, name string) (backup *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.CreateBackupByUUID: uuid=%s, poolID=%s, name=%s", uuid, poolID, name)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.CreateBackupByUUID: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.CreateBackupByUUID: success, backupID=%d", backup.ID)
		}
	}()
...
func (a *BackupAdmin) createBackup(ctx context.Context, volume *model.Volume, poolID string, name string) (backup *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.createBackup: volumeID=%d, poolID=%s, name=%s", volume.ID, poolID, name)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.createBackup: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.createBackup: success, backupID=%d", backup.ID)
		}
	}()
	memberShip := GetMemberShip(ctx)
	permit := memberShip.CheckResourceOrg(model.OrgWriter, volume.Owner)
	if !permit {
		logger.Ctx(ctx).Errorf("Not authorized to backup volume(%s)", volume.UUID)
		err = NewCLError(ErrPermissionDenied, "Not authorized to backup the volume", nil)
		return
	}
	if volume.Status == model.VolumeStatusError {
		logger.Ctx(ctx).Errorf("Volume %s is in error state, cannot backup now", volume.UUID)
	...
	if err != nil {
		logger.Ctx(ctx).Error("DB: query dictionary failed, storage_pool(%s) not found %+v", poolID, err)
	...
	if err != nil {
		logger.Ctx(ctx).Errorf("Failed to create backup record for volume(%s), %+v", volume.UUID, err)
	...
	if vol_driver != "local" {
		...
		logger.Ctx(ctx).Error("Backup not supported for local volume")
...
func (a *BackupAdmin) CreateSnapshotByID(ctx context.Context, volumeID int64, name string) (backup *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.CreateSnapshotByID: volumeID=%d, name=%s", volumeID, name)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.CreateSnapshotByID: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.CreateSnapshotByID: success, snapshotID=%d", backup.ID)
		}
	}()
	volume, err := volumeAdmin.Get(ctx, volumeID)
	if err != nil {
...
func (a *BackupAdmin) CreateSnapshotByUUID(ctx context.Context, uuid, name string) (backup *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.CreateSnapshotByUUID: uuid=%s, name=%s", uuid, name)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.CreateSnapshotByUUID: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.CreateSnapshotByUUID: success, snapshotID=%d", backup.ID)
		}
	}()
	volume, err := volumeAdmin.GetVolumeByUUID(ctx, uuid)
	if err != nil {
...
func (a *BackupAdmin) createSnapshot(ctx context.Context, name string, volume *model.Volume) (snapshot *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.createSnapshot: name=%s, volumeID=%d", name, volume.ID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.createSnapshot: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.createSnapshot: success, snapshotID=%d", snapshot.ID)
		}
	}()
	memberShip := GetMemberShip(ctx)
	permit := memberShip.CheckResourceOrg(model.OrgWriter, volume.Owner)
	if !permit {
		logger.Ctx(ctx).Error("Not authorized to snapshot volume")
		err = NewCLError(ErrPermissionDenied, "Not authorized to snapshot the volume", nil)
		return
	}
	if volume.Status == model.VolumeStatusError {
		logger.Ctx(ctx).Errorf("Volume %s is in error state, cannot snapshot now", volume.UUID)
	...
	if err != nil {
		logger.Ctx(ctx).Error("Failed to create snapshot", err)
	...
	vol_driver := GetVolumeDriver()
	uuid := volume.UUID
	logger.Ctx(ctx).Debugf("creating snapshot (%s) for volume %s", snapshot.UUID, uuid)
	if vol_driver != "local" {
		wdsUUID := volume.GetOriginVolumeID()
		wdsOriginPoolID := volume.GetVolumePoolID()
		...
		logger.Ctx(ctx).Error("Snapshot not supported for local volume")
...
func (a *BackupAdmin) createBackupModel(ctx context.Context, name, backupType string, volume *model.Volume, poolID string) (backup *model.VolumeBackup, task *model.Task, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.createBackupModel: name=%s, backupType=%s, volumeID=%d, poolID=%s", name, backupType, volume.ID, poolID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.createBackupModel: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.createBackupModel: success, backupID=%d, taskID=%d", backup.ID, task.ID)
		}
	}()
	ctx, db, newTransaction := StartTransaction(ctx)
	defer func() {
	...
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to create backup", err)
		return
	}
	...
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to create task", err)
		return
	}
	...
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to update task id to backup", err)
		return
	}
	return
}
...
func (a *BackupAdmin) GetBackupByID(ctx context.Context, backupID int64) (backup *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.GetBackupByID: backupID=%d", backupID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.GetBackupByID: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.GetBackupByID: success, backupUUID=%s", backup.UUID)
		}
	}()
	if backupID <= 0 {
		err_msg := fmt.Sprintf("Invalid backup ID: %d", backupID)
		logger.Ctx(ctx).Error(err_msg)
	...
	if err = db.Preload("Volume").Take(backup).Error; err != nil {
		logger.Ctx(ctx).Errorf("Failed to query backup/snapshot, %v", err)
	...
	if !permit {
		logger.Ctx(ctx).Error("Not authorized to read the backup")
...
func (a *BackupAdmin) GetBackupByUUID(ctx context.Context, uuID string) (backup *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.GetBackupByUUID: uuID=%s", uuID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.GetBackupByUUID: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.GetBackupByUUID: success, backupID=%d", backup.ID)
		}
	}()
	db := DB()
	memberShip := GetMemberShip(ctx)
...
func (a *BackupAdmin) Delete(ctx context.Context, backup *model.VolumeBackup) (err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.Delete: backupID=%d, backupUUID=%s", backup.ID, backup.UUID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.Delete: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT BackupAdmin.Delete: success")
		}
	}()
	if !backup.CanDelete() {
	...
	if !permit {
		logger.Ctx(ctx).Error("Not authorized to delete the backup")
		err = NewCLError(ErrPermissionDenied, "Not authorized to delete the backup", nil)
		return
	}
	err = db.Delete(backup).Error
	if err != nil {
		logger.Ctx(ctx).Error("DB: delete backup failed", err)
...
func (a *BackupAdmin) DeleteByID(ctx context.Context, backupID int64) (err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.DeleteByID: backupID=%d", backupID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.DeleteByID: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT BackupAdmin.DeleteByID: success")
		}
	}()
	backup, err := a.GetBackupByID(ctx, backupID)
	if err != nil {
		logger.Ctx(ctx).Error("Failed to get backup", err)
		err = NewCLError(ErrBackupNotFound, "Backup/Snapshot not found", err)
		return
	}
...
func (a *BackupAdmin) DeleteByUUID(ctx context.Context, uuID string) (err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.DeleteByUUID: uuID=%s", uuID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.DeleteByUUID: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT BackupAdmin.DeleteByUUID: success")
		}
	}()
...
func (a *BackupAdmin) Restore(ctx context.Context, backupID int64) (backup *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.Restore: backupID=%d", backupID)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.Restore: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.Restore: success, taskID=%d", backup.TaskID)
		}
	}()
	backup, err = a.GetBackupByID(ctx, backupID)
	if err != nil {
	...
	permit := memberShip.CheckResourceOrg(model.OrgWriter, volume.Owner)
	if !permit {
		logger.Ctx(ctx).Errorf("Not authorized to restore volume(%s)", volume.UUID)
	...
	err = db.Create(task).Error
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to create restore task", err)
		return
	}
	// update backup
	err = db.Model(&backup).Updates(map[string]interface{}{"task_id": task.ID}).Error
	if err != nil {
		logger.Ctx(ctx).Error("DB failed to update backup task", err)
		return
	}
	control := "inter="
	if volume.InstanceID > 0 {
	...
	if vol_driver != "local" {
		...
		logger.Ctx(ctx).Error("Restore not supported for local volume")
...
func (a *BackupAdmin) List(ctx context.Context, offset, limit int64, order, query string, volumeID int64, backupType string) (total int64, backups []*model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.List: offset=%d, limit=%d, order=%s, query=%s, volumeID=%d, backupType=%s", offset, limit, order, query, volumeID, backupType)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.List: error=%v", err)
		} else {
			logger.Ctx(ctx).Infof("EXIT BackupAdmin.List: total=%d, count=%d", total, len(backups))
		}
	}()
	memberShip := GetMemberShip(ctx)
	db := DB()
...
func (a *BackupAdmin) Update(ctx context.Context, id int64, name, path string, status model.BackupStatus) (backup *model.VolumeBackup, err error) {
	logger.Ctx(ctx).Infof("ENTER BackupAdmin.Update: id=%d, name=%s, path=%s, status=%s", id, name, path, status)
	defer func() {
		if err != nil {
			logger.Ctx(ctx).Errorf("EXIT BackupAdmin.Update: error=%v", err)
		} else {
			logger.Ctx(ctx).Info("EXIT BackupAdmin.Update: success")
		}
	}()
	ctx, db, newTransaction := StartTransaction(ctx)
	defer func() {
	...
	permit := memberShip.CheckResourceOrg(model.OrgWriter, backup.Owner)
	if !permit {
		logger.Ctx(ctx).Error("Not authorized to update the backup")
	...
	if err = db.Model(&model.VolumeBackup{}).Where("id = ?", backup.ID).Updates(map[string]interface{}{"name": backup.Name, "path": backup.Path, "status": backup.Status}).Error; err != nil {
		logger.Ctx(ctx).Error("DB: update backup failed", err)
