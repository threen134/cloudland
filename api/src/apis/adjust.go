...
type AdjustAPI struct {
	operator *services.AdjustOperator
}
...
func (a *AdjustAPI) CreateCPUAdjustRule(c *gin.Context) {
	...
	if len(req.Rules) == 0 {
		tracing.Logf(c, "[ADJUST-ERROR] No rules provided")
	...
	if len(req.Rules) > 1 {
		tracing.Logf(c, "[ADJUST-ERROR] Currently only one rule is supported")
	...
	if req.Owner != "admin" {
		tracing.Logf(c, "[ADJUST-ERROR] Permission denied: only admin can create adjustment rules")
	...
	for _, rule := range req.Rules {
		...
		if err := services.ProcessTemplate(c.Request.Context(), CPUAdjustRuleTemplate, fmt.Sprintf("cpu-adjust-%s-%s.yml", req.Owner, group.UUID), ruleData); err != nil {
			tracing.Logf(c, "Failed to render CPU adjust rule: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to render CPU adjust rule"})
			return
		}

		// Generate alert rules
		if err := services.ProcessTemplate(c.Request.Context(), ResourceAdjustAlertsTemplate, fmt.Sprintf("resource-adjust-alerts-%s-%s.yml", req.Owner, group.UUID), ruleData); err != nil {
			tracing.Logf(c, "Failed to render resource adjustment alerts: %v", err)
	...
	if len(req.LinkedVMs) > 0 {
		...
		alarmOperator := &services.AlarmOperator{}
		...
		if err := alarmOperator.BatchLinkVMs(c.Request.Context(), group.UUID, req.LinkedVMs, ""); err != nil {
			tracing.Logf(c, "[ADJUST-WARNING] Failed to link VMs: %v", err)
		}

		// Update matched_vms.json
		alarmAPI := &AlarmAPI{operator: &services.AlarmOperator{}}
		_ = alarmAPI.UpdateMatchedVMsJSON(c.Request.Context(), req.LinkedVMs, group.UUID, "add", "adjust-cpu")
	}

	// Reload Prometheus
	services.ReloadPrometheusViaHTTP(c.Request.Context())
...
func (a *AdjustAPI) GetCPUAdjustRules(c *gin.Context) {
	...
	if currentUser != "admin" {
		tracing.Logf(c, "[ADJUST-ERROR] Permission denied: only admin can view adjustment rules, user: %s", currentUser)
		c.JSON(http.StatusForbidden, gin.H{"error": "Permission denied: only admin can view adjustment rules"})
		return
	}

	queryParams := services.ListAdjustRuleGroupsParams{
...
func (a *AdjustAPI) DeleteCPUAdjustRule(c *gin.Context) {
	...
	if group.Owner != "admin" {
		tracing.Logf(c, "[ADJUST-ERROR] Permission denied: only admin can delete adjustment rules, owner: %s", group.Owner)
	...
	rulePath := fmt.Sprintf("%s/cpu-adjust-%s-%s.yml", services.RulesGeneral, group.Owner, group.UUID)
	alertPath := fmt.Sprintf("%s/resource-adjust-alerts-%s-%s.yml", services.RulesGeneral, group.Owner, group.UUID)

	// Determine symlink paths
	ruleLinkPath := fmt.Sprintf("%s/%s", services.RulesEnabled, filepath.Base(rulePath))
	alertLinkPath := fmt.Sprintf("%s/%s", services.RulesEnabled, filepath.Base(alertPath))

	// Delete symlinks and rule files
	deletedFiles := []string{}

	// Delete symlinks
	if err := services.RemoveFile(c.Request.Context(), ruleLinkPath); err == nil {
		deletedFiles = append(deletedFiles, ruleLinkPath)
		tracing.Logf(c, "[ADJUST-INFO] Removed symlink: %s", ruleLinkPath)
	} else {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to remove symlink %s: %v", ruleLinkPath, err)
	}

	if err := services.RemoveFile(c.Request.Context(), alertLinkPath); err == nil {
		deletedFiles = append(deletedFiles, alertLinkPath)
		tracing.Logf(c, "[ADJUST-INFO] Removed symlink: %s", alertLinkPath)
	} else {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to remove symlink %s: %v", alertLinkPath, err)
	}

	// Delete rule files
	if err := services.RemoveFile(c.Request.Context(), rulePath); err == nil {
		deletedFiles = append(deletedFiles, rulePath)
		tracing.Logf(c, "[ADJUST-INFO] Removed file: %s", rulePath)
	} else {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to remove file %s: %v", rulePath, err)
	}

	if err := services.RemoveFile(c.Request.Context(), alertPath); err == nil {
		deletedFiles = append(deletedFiles, alertPath)
		tracing.Logf(c, "[ADJUST-INFO] Removed file: %s", alertPath)
	} else {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to remove file %s: %v", alertPath, err)
	}

	// Restore CPU resources for all linked VMs
	tracing.Logf(c, "[ADJUST-INFO] Restoring CPU resources for all linked VMs before rule deletion: %s", group.UUID)
	vmLinks, err := alarmOperator.GetLinkedVMs(c.Request.Context(), group.UUID)
	if err != nil {
		tracing.Logf(c, "[ADJUST-WARNING] Failed to get linked VMs for CPU restore: %v", err)
	} else {
		// Restore CPU resources for each VM
		for _, link := range vmLinks {
			// Get VM domain information
			domain, err := services.GetDomainByInstanceUUID(c.Request.Context(), link.VMUUID)
			if err != nil {
				tracing.Logf(c, "[ADJUST-WARNING] Failed to get domain for VM %s: %v", link.VMUUID, err)
				continue
			}

			// Create restore record
			record := &services.AdjustmentRecord{
			...
			if err != nil {
				tracing.Logf(c, "[ADJUST-WARNING] Failed to restore CPU for VM %s: %v", link.VMUUID, err)
			} else {
				tracing.Logf(c, "[ADJUST-INFO] Successfully restored CPU for VM %s", link.VMUUID)
			}
		}
	}

	// Clean up adjustment status metrics on compute nodes
	tracing.Logf(c, "[ADJUST-INFO] Cleaning up CPU adjustment metrics for rule: %s", group.UUID)
	if err := a.cleanupRuleMetricsOnNodes(c.Request.Context(), group.UUID, "cpu"); err != nil {
		tracing.Logf(c, "[ADJUST-WARNING] Failed to cleanup rule metrics: %v", err)
	}
	...
	// Reload Prometheus
	tracing.Logf(c, "[ADJUST-INFO] Reloading Prometheus configuration")
	if err := services.ReloadPrometheusViaHTTP(c.Request.Context()); err != nil {
		tracing.Logf(c, "[ADJUST-WARNING] Failed to reload Prometheus: %v", err)
	}
...
func (a *AdjustAPI) ProcessResourceAdjustmentWebhook(c *gin.Context) {
	...
	if err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to read request body: %v", err)
	...
	// Parse alert data
	var req services.AlertWebhookRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to parse request body: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid alert format", "code": "INVALID_FORMAT"})
		return
	}

	tracing.Logf(c, "[ADJUST-INFO] Processing %d adjustment alert(s) with status: %s", len(req.Alerts), req.Status)
		...
		if notifyURL != "" {
			...
			if alert.Status == "firing" {
				...
				if adjustType != "" {
					...
					if status == 1 {
						tracing.Logf(c, "[ADJUST-INFO] Skip notification: domain=%s already limited", alert.Labels["domain"])
						continue
					}
				}
			}
			...
			notifyParams := services.NotifyParams{
				Alerts: []struct {
			...
			if err := alarmOperator.SendNotification(c.Request.Context(), notifyURL, notifyParams); err != nil {
				tracing.Logf(c, "[ADJUST-WARNING] Failed to send notification for alert %d: %v", i+1, err)
			} else {
				tracing.Logf(c, "[ADJUST-INFO] Successfully sent notification for domain: %s, action: %s, success: %v",
					alert.Labels["domain"], alert.Labels["action_type"], result)
			}
		} else {
			tracing.Logf(c, "[ADJUST-WARNING] No notify_url found in alert labels for alert %d", i+1)
		}
	}

	tracing.Logf(c, "[ADJUST-INFO] Adjustment completed: total=%d, success=%d, failed=%d",
...
func (a *AdjustAPI) queryAdjustmentStatus(ctx context.Context, domain, ruleID, adjustType, targetDevice string) int {
	...
	default:
		tracing.Logf(ctx, "[ADJUST-STATUS-QUERY] Invalid adjust type: %s", adjustType)
		return -1
	}

	client := &http.Client{Timeout: 5 * time.Second, Transport: tracing.HTTPTransport(nil)}
	req, err := http.NewRequestWithContext(ctx, "GET", PrometheusURL, nil)
	if err != nil {
		tracing.Logf(ctx, "[ADJUST-STATUS-QUERY] Failed to create request: %v", err)
		return -1
	}
	...
	if err != nil {
		tracing.Logf(ctx, "[ADJUST-STATUS-QUERY] Failed to query Prometheus: %v", err)
		return -1
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		tracing.Logf(ctx, "[ADJUST-STATUS-QUERY] Failed to decode response: %v", err)
		return -1
	}
	...
	tracing.Logf(ctx, "[ADJUST-STATUS-QUERY] Domain %s already limited (rule_id=%s, type=%s)", domain, ruleID, adjustType)
	return 1
}
...
func (a *AdjustAPI) queryBandwidthConfig(ctx context.Context, domain, targetDevice string) (bool, int, int) {
	query := fmt.Sprintf(`vm_interface_bandwidth_config_mbps{domain="%s",target_device="%s"}`, domain, targetDevice)
	tracing.Logf(ctx, "[BW-CONFIG-QUERY] Querying bandwidth config: domain=%s, device=%s, query=%s", domain, targetDevice, query)

	client := &http.Client{Timeout: 5 * time.Second, Transport: tracing.HTTPTransport(nil)}
	req, err := http.NewRequestWithContext(ctx, "GET", PrometheusURL, nil)
	if err != nil {
		tracing.Logf(ctx, "[BW-CONFIG-QUERY] Failed to create request: %v", err)
		return false, 0, 0
	}
	...
	resp, err := client.Do(req)
	if err != nil {
		tracing.Logf(ctx, "[BW-CONFIG-QUERY] Failed to query Prometheus: %v", err)
		return false, 0, 0
	}
	defer resp.Body.Close()
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		tracing.Logf(ctx, "[BW-CONFIG-QUERY] Failed to decode response: %v", err)
		return false, 0, 0
	}

	if result.Status != "success" || len(result.Data.Result) == 0 {
		tracing.Logf(ctx, "[BW-CONFIG-QUERY] No bandwidth config found for domain=%s, device=%s", domain, targetDevice)
		return false, 0, 0
	}
	...
	tracing.Logf(ctx, "[BW-CONFIG-QUERY] Retrieved config: domain=%s, device=%s, in=%d Mbps, out=%d Mbps",
		domain, targetDevice, inBw, outBw)
	return true, inBw, outBw
}

// processAlertAdjustment processes single alert adjustment logic
func (a *AdjustAPI) processAlertAdjustment(ctx context.Context, alert services.AdjustAlert, requestID string) bool {
	startTime := time.Now()
	...
	if domain == "" || ruleID == "" || actionType == "" {
		tracing.Logf(ctx, "[ADJUST-%s] Missing required parameters: domain=%s, ruleID=%s, actionType=%s",
			requestID, domain, ruleID, actionType)
		return false
	}

	tracing.Logf(ctx, "[ADJUST-%s] Processing %s for domain=%s, rule=%s", requestID, actionType, domain, ruleID)
	...
	if actionType == "limit_in_bw" || actionType == "restore_in_bw" ||
		...
		if ok, inBw, outBw := a.queryBandwidthConfig(ctx, domain, record.TargetDevice); ok {
			totalInBw = inBw
			totalOutBw = outBw
			tracing.Logf(ctx, "[ADJUST-%s] Bandwidth config: in=%d Mbps, out=%d Mbps", requestID, totalInBw, totalOutBw)
		} else {
			tracing.Logf(ctx, "[ADJUST-%s] Bandwidth config not found, treating as unlimited (0)", requestID)
			totalInBw = 0
			totalOutBw = 0
		}
	}
	...
	case "config_missing":
		...
		tracing.Logf(ctx, "[ADJUST-%s] Bandwidth config missing for VM %s interface %s - notification only, no DB record",
			requestID, domain, record.TargetDevice)
		return true // Return success as this is just a notification
	default:
		tracing.Logf(ctx, "[ADJUST-%s] Unknown adjustment type: %s", requestID, actionType)
	...
	if err != nil {
		tracing.Logf(ctx, "[ADJUST-%s] Failed: %v", requestID, err)
		history.Status = "failed"
		history.Details = fmt.Sprintf("Processing %s failed: %v", actionType, err)
	} else {
		tracing.Logf(ctx, "[ADJUST-%s] Completed successfully (%.2fs)", requestID, time.Since(startTime).Seconds())
...
func (a *AdjustAPI) CreateBWAdjustRule(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Parameter parsing failed: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	tracing.Logf(c, "[ADJUST-INFO] Creating bandwidth adjustment rule group: name=%s, type=%s", req.Name, ruleType)
	if err := a.operator.CreateAdjustRuleGroup(c.Request.Context(), group); err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to create rule group: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "create rule group failed: " + err.Error()})
		return
	}
	...
	for _, rule := range req.Rules {
		...
		if err := a.operator.CreateBWAdjustRuleDetail(c.Request.Context(), detail); err != nil {
			tracing.Logf(c, "[ADJUST-ERROR] Failed to create rule detail: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "create rule detail failed: " + err.Error()})
			return
		}
		...
		if err := services.ProcessTemplate(c.Request.Context(), template, filename, ruleData); err != nil {
			tracing.Logf(c, "[ADJUST-ERROR] Failed to render %s BW adjust rule: %v", rule.Direction, err)
	...
	if err := services.ProcessTemplate(c.Request.Context(), ResourceAdjustAlertsTemplate, fmt.Sprintf("resource-adjust-alerts-%s-%s.yml", req.Owner, group.UUID), alertRuleData); err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to render resource adjustment alerts: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to render resource adjustment alerts"})
		return
	}
	// Note: ProcessTemplate automatically creates symlink to rules_enabled (see alarm.go line 1390-1395)
	...
	if len(req.LinkedVMs) > 0 {
		...
		for i, vm := range req.LinkedVMs {
			if vm.VMUUID == "" {
				tracing.Logf(c, "[ADJUST-ERROR] Empty VM UUID at index %d", i)
		...
		if err != nil {
			tracing.Logf(c, "[ADJUST-WARN] Failed to get existing VM links for group %s: %v", group.UUID, err)
		} else {
		...
		for _, vm := range req.LinkedVMs {
			if err := alarmOperator.BatchLinkVMs(c.Request.Context(), group.UUID, []string{vm.VMUUID}, vm.TargetDevice); err != nil {
				tracing.Logf(c, "[ADJUST-WARNING] Failed to link VM %s: %v", vm.VMUUID, err)
			}
		}
...
func (a *AdjustAPI) GetBWAdjustRules(c *gin.Context) {
	...
	if groupUUID != "" {
		...
		// Query inbound and outbound bandwidth rules separately
		inBWParams := services.ListAdjustRuleGroupsParams{
			RuleType: model.RuleTypeAdjustInBW,
			Page:     page,
			PageSize: pageSize,
		}
		...
		outBWParams := services.ListAdjustRuleGroupsParams{
			RuleType: model.RuleTypeAdjustOutBW,
			Page:     page,
			PageSize: pageSize,
		}
	...
	for _, group := range groups {
		...
		if err != nil {
			tracing.Logf(c, "[ADJUST-ERROR] Failed to get bandwidth adjustment rule details: %v", err)
...
func (a *AdjustAPI) DeleteBWAdjustRule(c *gin.Context) {
	...
	details, err := a.operator.GetBWAdjustRuleDetails(c.Request.Context(), group.UUID)
	if err != nil {
		tracing.Logf(c, "[ADJUST-WARNING] Failed to get rule details for file cleanup: %v", err)
	...
	for _, detail := range details {
		...
		rulePath := fmt.Sprintf("%s/%s", services.RulesGeneral, filename)
		ruleLinkPath := fmt.Sprintf("%s/%s", services.RulesEnabled, filename)
	...
	tracing.Logf(c, "[ADJUST-INFO] Restoring bandwidth resources for all linked VMs before rule deletion: %s", group.UUID)
	vmLinks, err := alarmOperator.GetLinkedVMs(c.Request.Context(), group.UUID)
	if err != nil {
		tracing.Logf(c, "[ADJUST-WARNING] Failed to get linked VMs for bandwidth restore: %v", err)
	} else {
		...
		for _, link := range vmLinks {
			...
			if err != nil {
				tracing.Logf(c, "[ADJUST-WARNING] Failed to restore bandwidth for VM %s: %v", link.VMUUID, err)
			} else {
				tracing.Logf(c, "[ADJUST-INFO] Successfully restored bandwidth for VM %s", link.VMUUID)
			}
		}
	}

	// Clean up adjustment status metrics on compute nodes
	tracing.Logf(c, "[ADJUST-INFO] Cleaning up bandwidth adjustment metrics for rule: %s", group.UUID)
	...
	err = a.operator.DeleteAdjustRuleGroupWithDependencies(c.Request.Context(), group.UUID)
	if err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to delete rule group: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete rule: " + err.Error()})
		return
	}
...
func (a *AdjustAPI) LinkAdjustRule(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Invalid request parameters: %v", err)
	...
	if err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to get adjustment rule group: %v", err)
		c.JSON(http.StatusNotFound, gin.H{"error": "adjustment rule group not found"})
		return
	}

	tracing.Logf(c, "[ADJUST-INFO] Linking VM to adjustment rule: identifier=%s, vm_uuid=%s, interface=%s",
	...
	if group.Type == model.RuleTypeAdjustInBW || group.Type == model.RuleTypeAdjustOutBW {
		...
		if req.Interface == "" {
			tracing.Logf(c, "[ADJUST-ERROR] Interface parameter is required for bandwidth adjustment rules")
	...
	if exists {
		tracing.Logf(c, "[ADJUST-INFO] VM already linked to rule group, skipping: vm_uuid=%s, group_uuid=%s, interface=%s",
	...
	if err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to create VM link: %v", err)
	...
	if err != nil {
		tracing.Logf(c, "[ADJUST-WARN] Failed to update Prometheus config: %v", err)
		// Don't return error as database operation was successful
	}

	tracing.Logf(c, "[ADJUST-INFO] Successfully linked VM to adjustment rule: group_uuid=%s, vm_uuid=%s, interface=%s",
...
func (a *AdjustAPI) UnlinkAdjustRule(c *gin.Context) {
	...
	tracing.Logf(c, "[ADJUST-INFO] Unlinking VM from adjustment rule: identifier=%s, vm_uuid=%s, interface=%s",
		identifier, req.VMUUID, req.Interface)
	...
	existingLinks, err := alarmOperator.GetLinkedVMs(c.Request.Context(), group.UUID)
	if err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to get linked VMs: %v", err)
	...
	if !linkExists {
		if vmFoundWithDifferentInterface {
			...
			tracing.Logf(c, "[ADJUST-ERROR] VM linked with different interface: vm_uuid=%s, requested_interface=%s, actual_interface=%s",
			...
			tracing.Logf(c, "[ADJUST-WARN] VM not linked to rule group: vm_uuid=%s, group_uuid=%s, interface=%s",
				req.VMUUID, group.UUID, req.Interface)
	...
	// Get VM domain information
	domain, err := services.GetDomainByInstanceUUID(c.Request.Context(), req.VMUUID)
	vmExists := true
	if err != nil {
		tracing.Logf(c, "[ADJUST-WARN] Failed to get domain for VM %s (VM may have been deleted): %v", req.VMUUID, err)
		vmExists = false
	...
	if vmExists {
		tracing.Logf(c, "[ADJUST-INFO] Checking if VM is currently being limited: domain=%s", domain)
	} else {
		tracing.Logf(c, "[ADJUST-INFO] VM %s does not exist, skipping resource restoration", req.VMUUID)
	}
	...
	if vmExists {
		...
		case model.RuleTypeAdjustCPU:
			...
			tracing.Logf(c, "[ADJUST-INFO] Checking CPU adjustment status for domain: %s", domain)

			// Check CPU adjustment status metrics
			isCPULimited, err := a.checkVMAdjustmentStatus(c.Request.Context(), domain, "cpu", group.UUID)
			if err != nil {
				tracing.Logf(c, "[ADJUST-WARN] Failed to check CPU adjustment status: %v", err)
			} else if isCPULimited {
				tracing.Logf(c, "[ADJUST-INFO] VM is currently CPU limited, performing restore: domain=%s", domain)
				record.AdjustType = "restore_cpu"
				err = a.operator.RestoreCPUResource(c.Request.Context(), record, domain, req.VMUUID)
				if err != nil {
					tracing.Logf(c, "[ADJUST-WARN] Failed to restore CPU resources: %v", err)
				} else {
					tracing.Logf(c, "[ADJUST-INFO] Successfully restored CPU resources for domain: %s", domain)
				}
			} else {
				tracing.Logf(c, "[ADJUST-INFO] VM is not currently CPU limited: domain=%s", domain)
			}
		case model.RuleTypeAdjustInBW, model.RuleTypeAdjustOutBW:
			// Bandwidth type: check and restore bandwidth resources
			tracing.Logf(c, "[ADJUST-INFO] Checking bandwidth adjustment status for domain: %s, interface: %s", domain, req.Interface)
			...
			if err != nil {
				tracing.Logf(c, "[ADJUST-WARN] Failed to check bandwidth adjustment status: %v", err)
			} else if isBWLimited {
				tracing.Logf(c, "[ADJUST-INFO] VM is currently bandwidth limited, performing restore: domain=%s, interface=%s", domain, req.Interface)
				...
				if err != nil {
					tracing.Logf(c, "[ADJUST-WARN] Failed to restore bandwidth resources: %v", err)
				} else {
					tracing.Logf(c, "[ADJUST-INFO] Successfully restored bandwidth resources for domain: %s, interface: %s", domain, req.Interface)
				}
			} else {
				tracing.Logf(c, "[ADJUST-INFO] VM is not currently bandwidth limited: domain=%s, interface=%s", domain, req.Interface)
			}
		}
	} else {
		tracing.Logf(c, "[ADJUST-INFO] Skipping resource restoration for deleted VM: %s", req.VMUUID)
	}

	// Delete link
	_, err = alarmOperator.DeleteVMLink(c.Request.Context(), group.UUID, req.VMUUID, req.Interface)
	if err != nil {
		tracing.Logf(c, "[ADJUST-ERROR] Failed to delete VM link: %v", err)
	...
	tracing.Logf(c, "[ADJUST-INFO] Cleaning up %s adjustment metrics for rule: %s", ruleType, group.UUID)
	if err := a.cleanupRuleMetricsOnNodes(c.Request.Context(), group.UUID, ruleType); err != nil {
		tracing.Logf(c, "[ADJUST-WARN] Failed to cleanup rule metrics: %v", err)
		// Don't return error as database operation was successful
	}
	...
	tracing.Logf(c, "[ADJUST-INFO] Successfully unlinked VM from adjustment rule: group_uuid=%s, vm_uuid=%s, interface=%s",
		group.UUID, req.VMUUID, req.Interface)
...
func (a *AdjustAPI) checkVMAdjustmentStatus(ctx context.Context, domain, metricType, ruleGroupUUID string) (bool, error) {
	...
	tracing.Logf(ctx, "[ADJUST-INFO] Querying Prometheus: %s", req.URL.String())
...
func (a *AdjustAPI) cleanupVMMetrics(ctx context.Context, vmUUID, ruleGroupUUID string, ruleType string) error {
	...
	instance, err := services.GetInstanceByUUIDWithAuth(ctx, vmUUID, nil)
	if err != nil {
		return fmt.Errorf("failed to get instance info: %v", err)
	}

	// Get VM domain information
	domain, err := services.GetDomainByInstanceUUID(ctx, vmUUID)
	if err != nil {
	...
	case model.RuleTypeAdjustInBW, model.RuleTypeAdjustOutBW:
		...
		tracing.Logf(ctx, "[BW-STATUS-CLEANUP] Full command: %s", command)
...
func (a *AdjustAPI) GetRuleLinks(c *gin.Context) {
	...
	if ruleID == "" {
		tracing.Logf(c, "[RULE-ERROR] Missing rule_id parameter")
		c.JSON(http.StatusBadRequest, gin.H{"error": "rule_id parameter is required"})
		return
	}

	tracing.Logf(c, "[RULE-INFO] Getting rule links: rule_id=%s", ruleID)
	...
	if err == nil {
		...
		ruleSource = "adjust"
		found = true
		tracing.Logf(c, "[RULE-INFO] Found adjustment rule: rule_id=%s, uuid=%s, type=%s", ruleID, groupUUID, ruleType)
	}
	...
	if !found {
		...
		if err == nil {
			...
			ruleSource = "alarm"
			found = true
			tracing.Logf(c, "[RULE-INFO] Found alarm rule: rule_id=%s, uuid=%s, type=%s", ruleID, groupUUID, ruleType)
		}
	}

	// If still not found, return error
	if !found {
		tracing.Logf(c, "[RULE-ERROR] Rule not found: rule_id=%s", ruleID)
	...
	if err != nil {
		tracing.Logf(c, "[RULE-ERROR] Failed to get linked VMs: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to get linked VMs: " + err.Error()})
		return
	}
	...
	for _, link := range vmLinks {
		...
		domain, err := services.GetDomainByInstanceUUID(c.Request.Context(), link.VMUUID)
		if err != nil {
			tracing.Logf(c, "[RULE-WARN] Failed to get domain for VM %s: %v", link.VMUUID, err)
			domain = "unknown"
		}
	...
	tracing.Logf(c, "[RULE-INFO] Retrieved rule links: rule_id=%s, source=%s, type=%s, link_count=%d",
...
func (a *AdjustAPI) cleanupRuleMetricsOnNodes(ctx context.Context, ruleGroupUUID, ruleType string) error {
	tracing.Logf(ctx, "[ADJUST-INFO] Starting cleanup of %s metrics for rule group: %s", ruleType, ruleGroupUUID)

	// Get list of VMs associated with this rule group
	alarmOperator := &services.AlarmOperator{}
	vmLinks, err := alarmOperator.GetLinkedVMs(ctx, ruleGroupUUID)
	if err != nil {
		tracing.Logf(ctx, "[ADJUST-ERROR] Failed to get linked VMs for rule cleanup: %v", err)
		return fmt.Errorf("failed to get linked VMs: %v", err)
	}

	if len(vmLinks) == 0 {
		tracing.Logf(ctx, "[ADJUST-INFO] No VMs linked to rule group %s, no metrics to cleanup", ruleGroupUUID)
		return nil
	}

	tracing.Logf(ctx, "[ADJUST-INFO] Found %d VMs linked to rule group %s", len(vmLinks), ruleGroupUUID)
	...
	for _, link := range vmLinks {
		...
		if err != nil {
			tracing.Logf(ctx, "[ADJUST-WARNING] Failed to get instance info for VM %s: %v", link.VMUUID, err)
			continue
		}
		if instance.Hyper > 0 {
	...
	if len(hyperNodes) == 0 {
		tracing.Logf(ctx, "[ADJUST-WARNING] No valid compute nodes found for rule group %s", ruleGroupUUID)
		return nil
	}

	tracing.Logf(ctx, "[ADJUST-INFO] Will cleanup metrics on %d compute nodes", len(hyperNodes))
	...
	for hyperID := range hyperNodes {
		...
		if err != nil {
			...
			tracing.Logf(ctx, "[ADJUST-ERROR] %s", errMsg)
			cleanupErrors = append(cleanupErrors, errMsg)
		} else {
			tracing.Logf(ctx, "[ADJUST-INFO] Successfully cleaned up %s metrics on compute node %d", ruleType, hyperID)
		}
	}
	...
	tracing.Logf(ctx, "[ADJUST-INFO] Successfully cleaned up %s metrics for rule group %s on all compute nodes", ruleType, ruleGroupUUID)
	return nil
}
...
func (a *AdjustAPI) RegenerateBandwidthConfigMetrics(c *gin.Context) {
	...
	query := dbs.DBContext(ctx).Preload("Interfaces").Where("status = ?", "active")

	if hyperIDStr != "" {
		// Filter by specific hyper node if provided
		hyperID, err := strconv.Atoi(hyperIDStr)
		if err != nil {
			tracing.Logf(ctx, "[ADJUST-ERROR] Invalid hyper_id parameter: %v", err)
			c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid hyper_id parameter"})
			return
		}
		query = query.Where("hyper = ?", hyperID)
		tracing.Logf(ctx, "[ADJUST-INFO] Regenerating bandwidth config metrics for hyper node %d", hyperID)
	} else {
		tracing.Logf(ctx, "[ADJUST-INFO] Regenerating bandwidth config metrics for all instances")
	}

	if err := query.Find(&instances).Error; err != nil {
		tracing.Logf(ctx, "[ADJUST-ERROR] Failed to query instances: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to query instances"})
		return
	}

	tracing.Logf(ctx, "[ADJUST-INFO] Found %d active instances to process", len(instances))
	...
	tracing.Logf(ctx, "[ADJUST-INFO] Grouped interfaces across %d hyper nodes", len(hyperInterfaces))
	...
	for hyperID, interfaces := range hyperInterfaces {
		tracing.Logf(ctx, "[ADJUST-INFO] Processing hyper %d with %d interfaces", hyperID, len(interfaces))
		...
		for _, ifaceInfo := range interfaces {
			...
			if err := a.operator.UpdateVMBandwidthMetric(ctx, hyperID, ifaceInfo.Domain, ifaceInfo.TargetDevice,
				...
				tracing.Logf(ctx, "[ADJUST-ERROR] Failed to update metric for %s/%s on hyper %d: %v",
		...
		if lastErr != nil {
			...
			tracing.Logf(ctx, "[ADJUST-INFO] Successfully updated bandwidth config metrics on hyper %d (%d interfaces)",
...
func (a *AdjustAPI) PatchCPUAdjustRule(c *gin.Context) {
	...
	if err != nil {
		...
		tracing.Logf(c, "[PATCH-ERROR] Failed to query rule group: %v", err)
	...
	if len(updates) > 0 {
		if err := a.operator.UpdateAdjustRuleGroupBasicInfo(c.Request.Context(), group.UUID, updates); err != nil {
			tracing.Logf(c, "[PATCH-ERROR] Failed to update rule group: %v", err)
	...
	if err != nil {
		tracing.Logf(c, "[PATCH-ERROR] Failed to query old rule details: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to query old rule details"})
		return
	}
	if len(oldDetails) == 0 {
		tracing.Logf(c, "[PATCH-ERROR] Data inconsistency: rule has no details (rule_id=%s, group_uuid=%s)", group.RuleID, group.UUID)
	...
	if len(oldDetails) > 1 {
		tracing.Logf(c, "[PATCH-ERROR] Data inconsistency: rule has multiple details (rule_id=%s, group_uuid=%s, count=%d)", group.RuleID, group.UUID, len(oldDetails))
		c.JSON(http.StatusInternalServerError, gin.H{
	...
	if len(req.Rules) > 0 {
		...
		if err := a.operator.UpdateCPUAdjustRuleDetails(c.Request.Context(), identifier, []model.CPUAdjustRuleDetail{detail}); err != nil {
			tracing.Logf(c, "[PATCH-ERROR] Failed to update rule details: %v", err)
	...
	if req.LinkedVMs != nil {
		...
		if err != nil {
			tracing.Logf(c, "[PATCH-ERROR] Failed to sync VM links: %v", err)
	...
	if err := services.ProcessTemplate(c.Request.Context(), CPUAdjustRuleTemplate, fmt.Sprintf("cpu-adjust-%s-%s.yml", group.Owner, group.UUID), ruleData); err != nil {
		tracing.Logf(c, "[PATCH-ERROR] Failed to render CPU adjust rule: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to render CPU adjust rule"})
		return
	}

	// Render alert rules
	if err := services.ProcessTemplate(c.Request.Context(), ResourceAdjustAlertsTemplate, fmt.Sprintf("resource-adjust-alerts-%s-%s.yml", group.Owner, group.UUID), ruleData); err != nil {
		tracing.Logf(c, "[PATCH-ERROR] Failed to render resource adjustment alerts: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to render resource adjustment alerts"})
		return
	}
	...
	if len(toRemove) > 0 {
		_ = services.UpdateMatchedVMsJSON(c.Request.Context(), toRemove, group.UUID, "remove", "adjust-cpu")
	}
	if len(toAdd) > 0 {
		_ = services.UpdateMatchedVMsJSON(c.Request.Context(), toAdd, group.UUID, "add", "adjust-cpu")
	}

	// 9. Reload Prometheus
	if err := services.ReloadPrometheusViaHTTP(c.Request.Context()); err != nil {
		tracing.Logf(c, "[PATCH-WARNING] Failed to reload Prometheus: %v", err)
...
func (a *AdjustAPI) PatchBWAdjustRule(c *gin.Context) {
	...
	if oldDirection != "" && newDirection != "" && oldDirection != newDirection {
		tracing.Logf(c, "[PATCH-INFO] Direction changed from %s to %s, cleaning up old file", oldDirection, newDirection)
		...
		if oldFilename != "" {
			oldLinkPath := filepath.Join(services.RulesEnabled, oldFilename)
			oldRulePath := filepath.Join(services.RulesGeneral, oldFilename)

			_ = services.RemoveFile(c.Request.Context(), oldLinkPath)
			_ = services.RemoveFile(c.Request.Context(), oldRulePath)
			tracing.Logf(c, "[PATCH-INFO] Cleaned up old files: %s, %s", oldLinkPath, oldRulePath)
		}
	}
	...
	if len(req.Rules) > 0 {
		...
		if err != nil || len(updatedDetails) == 0 {
			tracing.Logf(c, "[PATCH-ERROR] Failed to get updated detail: %v", err)
	...
	// Render recording rules
	if err := services.ProcessTemplate(c.Request.Context(), templateFile, outputFile, ruleData); err != nil {
		tracing.Logf(c, "[PATCH-ERROR] Failed to render BW adjust rule: %v", err)
	...
	for device, vmUUIDs := range toRemoveByDevice {
		_ = services.UpdateMatchedVMsJSON(c.Request.Context(), vmUUIDs, group.UUID, "remove", "adjust-bw", device)
	}
	for device, vmUUIDs := range toAddByDevice {
		_ = services.UpdateMatchedVMsJSON(c.Request.Context(), vmUUIDs, group.UUID, "add", "adjust-bw", device)
	}
