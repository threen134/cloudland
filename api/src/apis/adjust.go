...
func (a *AdjustAPI) CreateCPUAdjustRule(c *gin.Context) {
	...
	for _, rule := range req.Rules {
		...
		if err := services.ProcessTemplate(CPUAdjustRuleTemplate, fmt.Sprintf("cpu-adjust-%s-%s.yml", req.Owner, group.UUID), ruleData); err != nil {
		...
		if err := services.ProcessTemplate(ResourceAdjustAlertsTemplate, fmt.Sprintf("resource-adjust-alerts-%s-%s.yml", req.Owner, group.UUID), ruleData); err != nil {
...
func (a *AdjustAPI) DeleteCPUAdjustRule(c *gin.Context) {
	...
	// Delete symlinks
	if err := services.RemoveFile(ruleLinkPath); err == nil {
	...
	if err := services.RemoveFile(alertLinkPath); err == nil {
	...
	if err := services.RemoveFile(alertPath); err == nil {
	...
	if err := services.ReloadPrometheusViaHTTP(); err != nil {
...
func (a *AdjustAPI) CreateBWAdjustRule(c *gin.Context) {
	...
	for _, rule := range req.Rules {
		...
		if err := services.ProcessTemplate(template, filename, ruleData); err != nil {
	...
	if err := services.ProcessTemplate(ResourceAdjustAlertsTemplate, fmt.Sprintf("resource-adjust-alerts-%s-%s.yml", req.Owner, group.UUID), alertRuleData); err != nil {
...
func (a *AdjustAPI) PatchCPUAdjustRule(c *gin.Context) {
	...
	if err := services.ProcessTemplate(CPUAdjustRuleTemplate, fmt.Sprintf("cpu-adjust-%s-%s.yml", group.Owner, group.UUID), ruleData); err != nil {
	...
	// Render alert rules
	if err := services.ProcessTemplate(ResourceAdjustAlertsTemplate, fmt.Sprintf("resource-adjust-alerts-%s-%s.yml", group.Owner, group.UUID), ruleData); err != nil {
...
func (a *AdjustAPI) PatchBWAdjustRule(c *gin.Context) {
	...
	if oldDirection != "" && newDirection != "" && oldDirection != newDirection {
		...
		if oldFilename != "" {
			...
			_ = services.RemoveFile(oldLinkPath)
			_ = services.RemoveFile(oldRulePath)
