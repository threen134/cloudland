...
func BackupVolumeWDSVhost(ctx context.Context, args []string) (status string, err error) {
	...
	logger.Ctx(ctx).Debug("BackupVolumeWDSVhost", args)
	if len(args) < 8 {
		logger.Ctx(ctx).Errorf("Invalid args for create_snapshot_wds_vhost: %v", args)
	...
	if err != nil {
		logger.Ctx(ctx).Errorf("Invalid backup ID: %v", args[1])
		return
	}
	size, err := strconv.ParseInt(args[5], 10, 64)
	if err != nil {
		logger.Ctx(ctx).Errorf("Invalid backup/snapshot size: %v", args[5])
		size = 0
	}
	if size > 0 {
	...
	if err != nil {
		logger.Ctx(ctx).Error("Invalid backup ID", err)
		return
	}
	...
	if err != nil {
		logger.Ctx(ctx).Errorf("Failed to update backup %d: %v", backupID, err)
		return "", err
	}
	...
	if err != nil {
		logger.Ctx(ctx).Errorf("Failed to update volume %d: %v", volume.ID, err)
		return "", err
	}
	return
}
...
func RestoreVolumeWDSVhost(ctx context.Context, args []string) (status string, err error) {
	...
	logger.Ctx(ctx).Debug("restore_snapshot_wds_vhost", args)
	if len(args) < 6 {
		logger.Ctx(ctx).Errorf("Invalid args for restore_snapshot_wds_vhost: %v", args)
		err = fmt.Errorf("wrong params")
		return
	}
	taskID, err := strconv.ParseInt(args[1], 10, 64)
	...
	backupID, err := strconv.ParseInt(args[2], 10, 64)
	if err != nil {
		logger.Ctx(ctx).Errorf("Invalid backup ID: %v", args[2])
		return
	}
	volumeID, err := strconv.ParseInt(args[3], 10, 64)
	if err != nil {
		logger.Ctx(ctx).Errorf("Invalid volume ID: %v", args[3])
		return
	}
	status = args[4]
	path := args[5]
	...
	if err != nil {
		logger.Ctx(ctx).Errorf("Failed to update volume %d: %v", volumeID, err)
		return "", err
	}
	return
}
