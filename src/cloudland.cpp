...
void usage(char *pName) {
  printf("%s [-p pidDir] [-l logDir] [-s severity] [-c hostFile] [-e] \n\n",
         pName);
  printf("    -p pidDir\t\tSpecify the pid directory. Default is: "
         "\"/var/run/\"(Linux) and \"/var/opt/\"(AIX)\n");
  printf("    -l logDir\t\tSpecify the log directory. Default is: "
         "\"/var/log/\"\n");
  printf("    -s severity\t\tSpecify the log severity. Default is: "
         "\"'3'(INFORMATION)\"\n");
...
int main(int argc, char *argv[]) {
  log_info("Cloudland control plane starting...");
  try {
    if (initParams(argc, argv) != 0) {
      log_error("Failed to initialize parameters, exiting.");
      return -1;
    }

    log_info("Initializing RpcWorker...");
    RpcWorker worker;
    log_info("Running RpcWorker server...");
    worker.runServer();
    log_info("Cloudland control plane exiting normally.");
  } catch (const CommonException &e) {
    log_crit("Caught CommonException: %s", e.getErrMsg());
    return -1;
  } catch (const std::exception &e) {
    log_crit("Caught std::exception: %s", e.what());
    return -1;
  } catch (...) {
