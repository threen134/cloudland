...
ResourceManager::ResourceManager() : updated(false) {
...
double ResourceManager::testResource(Resource *resc, long cpu, long memory,
                                     long disk, long network) {
  double current = 0.0;
...
int ResourceManager::getBestBranch(long cpu, long memory, long disk,
                                   long network, int group) {
  int bestID = -1;
  static int count = 0;
  double current = 0.0;
  int i, j, idx;
...
int ResourceManager::setAvailibility(int id, long cpu, long total_cpu,
                                     long memory, long total_memory, long disk,
                                     long total_disk, long network,
                                     long total_network, long load,
                                     long total_load) {
...
int ResourceManager::getTotalMsg(char *rcMsg, int size) {
  memset(rcMsg, '\0', size);
  snprintf(rcMsg, size - 1,
           "report_rc.sh 'cpu=%ld/%ld' 'memory=%ld/%ld' 'disk=%ld/%ld' "
           "'network=%ld/%ld' 'load=%ld/%ld'\n",
           totalRC.cpu, totalRC.total_cpu, totalRC.memory, totalRC.total_memory,
           totalRC.disk, totalRC.total_disk, totalRC.network,
           totalRC.total_network, totalRC.load, totalRC.total_load);
