...
void *report_thread(void *param) {
  ...
  fprintf(stderr,
          "[SCHEDULER] report_thread started: agent_id=%d listening on %s:%d\n",
          myID, hostname, port);
  ...
  fprintf(stderr,
          "[SCHEDULER] report_thread agent id=%d exiting with status=%d\n",
          myID, status);
...
int filter_initialize(void **user_param) {
  ...
  *user_param = rcManager;
  ...
  if (rc != 0) {
    fprintf(stderr,
            "[SCHEDULER] filter_initialize: pthread_create failed rc=%d\n", rc);
...
int bcast_message(int msgID, int group, void *buffer, int size) {
  ...
  if (num <= 0) {
    fprintf(stderr,
            "[SCHEDULER] bcast_message: no successors in group, msgID=%d\n",
            msgID);
    return rc;
  }
  ...
  if (rc != SCI_SUCCESS) {
    fprintf(
        stderr,
        "[SCHEDULER] bcast_message: SCI_Filter_bcast failed rc=%d msgID=%d\n",
        rc, msgID);
  }
  delete[] children;
...
int upload_message(int msgID, int myID, int group, const char *control,
  ...
  if (rc != SCI_SUCCESS) {
    fprintf(
        stderr,
        "[SCHEDULER] upload_message failed rc=%d msgID=%d myID=%d control=%s\n",
...
int upload_topology(int *children, int num, int *recovery, int rnum, int myID,
                    int msgID) {
  ...
  control += tmp;
  upload_message(msgID, myID, SCI_GROUP_ALL, (char *)control.c_str(),
                 (char *)message.c_str());
...
int filter_input(void *user_param, sci_group_t group, void *buffer, int size) {
  ...
  if (report != NULL) {
    ...
    rcManager->setAvailibility(beID, cpu, total_cpu, memory, total_memory, disk,
                               total_disk, network, total_network, load,
                               total_load);
    ...
    if (target < 0) {
      ...
      if (bestID != -1) {
        fprintf(stderr,
                "[SCHEDULER] filter_input: scheduled msgID=%d to node %d"
                " (cpu=%ld memory=%ld disk=%ld)\n",
                msgID, bestID, cpu, memory, disk);
        rc = SCI_Filter_bcast(SCHEDULE_FILTER, 1, &bestID, 1, &buffer, &size);
        if (rc != SCI_SUCCESS) {
          fprintf(stderr,
                  "[SCHEDULER] filter_input: SCI_Filter_bcast to node %d "
                  "failed rc=%d\n",
                  bestID, rc);
        }
      } else {
        fprintf(stderr,
                "[SCHEDULER] filter_input: no available node for msgID=%d"
